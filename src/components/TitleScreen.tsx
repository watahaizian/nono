...
const TitleScreen: React.FC<TitleScreenProps> = ({ onStart, onEdit }) => {
  return (
    <div className="relative flex flex-col items-center justify-center h-screen bg-gradient-to-r from-blue-500 to-purple-600 text-white">
      ...
      <button className="bg-white text-blue-500 font-semibold py-2 px-4 rounded-lg shadow-md hover:bg-gray-200 transition duration-300" onClick={onEdit}>
        作成
      </button>
      {/* バージョン番号を左下に表示 */}
      <div className="absolute left-2 bottom-2 text-sm text-white opacity-75">Version {version}</div>
    </div>
  );
};
